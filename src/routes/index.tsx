    ...
    <AppShell>
      ...
      <section className="relative overflow-hidden bg-gradient-hero px-5 pt-6 pb-10">
        ...
        <div className="relative">
          <p className="text-xs uppercase tracking-[0.25em] text-gold/80">{t("home", "hero_kicker")}</p>
          <h1 className="mt-2 font-display text-[2.1rem] leading-[1.05] text-foreground">
            {renderHighlighted(t("home", "hero_title"))}
          </h1>

          <div className="mt-6 flex items-center justify-center">
            <div className="relative">
              <div className="absolute inset-0 rounded-full bg-gold/20 blur-2xl" />
              <img src={heroLogo} alt="" className="relative h-28 w-auto drop-shadow-2xl" />
            </div>
          </div>
          ...
          >
            <div>
              <div className="text-[10px] uppercase tracking-widest text-gold">{t("home", "next_service_label")}</div>
              <div className="mt-1 font-display text-lg">{t("home", "next_service_when")}</div>
              <div className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
                <MapPin className="h-3 w-3" /> {t("home", "next_service_place")}
              </div>
            </div>
      ...
      <section className="px-5 -mt-2">
        <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
          <div className="flex items-center justify-between">
            <span className="text-[10px] uppercase tracking-widest text-gold">{t("home", "verse_label")}</span>
            <BookOpen className="h-4 w-4 text-gold" />
          </div>
          <p className="mt-3 font-display text-lg leading-snug text-foreground">
            “{t("home", "verse_text")}”
          </p>
          <p className="mt-2 text-xs text-muted-foreground">{t("home", "verse_ref")}</p>
        </div>
      </section>

      <section className="px-5 mt-6">
        <h2 className="font-display text-xl mb-3">{t("home", "quick_title")}</h2>
      ...
      <section className="px-5 mt-7">
        <div className="flex items-end justify-between mb-3">
          <h2 className="font-display text-xl">{t("home", "sermon_section_title")}</h2>
          ...
          <div className="relative h-40 bg-gradient-hero">
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-center justify-between">
              <span className="rounded-full bg-background/70 px-2.5 py-1 text-[10px] uppercase tracking-widest text-gold backdrop-blur">
                {t("home", "sermon_card_tag")}
              </span>
              <span className="rounded-full bg-background/70 px-2 py-1 text-[10px] text-muted-foreground backdrop-blur">
                {t("home", "sermon_card_duration")}
              </span>
            </div>
          </div>
          <div className="p-4">
            <h3 className="font-display text-lg leading-tight">{t("home", "sermon_card_title")}</h3>
            <p className="mt-1 text-xs text-muted-foreground">{t("home", "sermon_card_meta")}</p>
          </div>
        </Link>
      </section>
      ...
      <section className="px-5 mt-7">
        <div className="rounded-2xl border border-border bg-gradient-hero p-5 shadow-card">
          <span className="text-[10px] uppercase tracking-widest text-gold">{t("home", "devo_label")}</span>
          <h3 className="mt-2 font-display text-xl">{t("home", "devo_title")}</h3>
      ...
      <section className="px-5 mt-7">
        <h2 className="font-display text-xl mb-3">{t("home", "community_title")}</h2>
